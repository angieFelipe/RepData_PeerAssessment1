...
```{r central_s_steps}
meanssteps <- round(mean(hist1$s_steps),2)
medianssteps <- round(median(hist1$s_steps),2)
...
with(hist2, hist(s_steps))
meanssteps2 <- round(mean(hist2$s_steps),2)
medianssteps2 <- round(median(hist2$s_steps),2)

```

The mean of total number of steps taken per day is now `r meanssteps2` while the median of
steps are now `r medianssteps2`

*Do these values differ from the estimates from the first part of the assignment? *

```{r comparing_central}
with_na <- c(mean=meanssteps, median = medianssteps)
without_na <- c(mean=meanssteps2, median = medianssteps2)

table<-cbind(with_na,without_na)
table

```
 Both, mean and median have increase to the same value `r meanssteps2` 
